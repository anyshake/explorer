...
void gnss_model_setup(bool is_rtos) {
    ...
    const char set_gll_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x01, 0x00, 0x00, 0x0D};
    mcu_utils_uart2_write((uint8_t*)set_gll_disabled, sizeof(set_gll_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);

    // disable GSA messages
    const char set_gsa_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x02, 0x00, 0xFC, 0x13};
    mcu_utils_uart2_write((uint8_t*)set_gsa_disabled, sizeof(set_gsa_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);

    // disable GRS messages
    const char set_grs_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x03, 0x00, 0xFD, 0x15};
    mcu_utils_uart2_write((uint8_t*)set_grs_disabled, sizeof(set_grs_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);

    // disable GSV messages
    const char set_gsv_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x04, 0x00, 0xFE, 0x17};
    mcu_utils_uart2_write((uint8_t*)set_gsv_disabled, sizeof(set_gsv_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);

    // disable VTG messages
    const char set_vtg_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x06, 0x00, 0x00, 0x1B};
    mcu_utils_uart2_write((uint8_t*)set_vtg_disabled, sizeof(set_vtg_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);

    // disable ZDA messages
    const char set_zda_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x07, 0x00, 0x01, 0x1D};
    mcu_utils_uart2_write((uint8_t*)set_zda_disabled, sizeof(set_zda_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);

    // disable GST messages
    const char set_gst_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x08, 0x00, 0x02, 0x1F};
    mcu_utils_uart2_write((uint8_t*)set_gst_disabled, sizeof(set_gst_disabled), true);
    mcu_utils_delay_ms(200, is_rtos);
...
// reference: https://web.archive.org/web/20250812142418/https://wiki.millerjs.org/atgm336h
...
// reference: https://web.archive.org/web/20250812142858if_/https://micronika.ru/upload/iblock/665/4c6k4oyd63bikyc9c33jgbz19kc1aois.pdf

void gnss_model_setup(bool is_rtos) {
    // disable ZDA message
    const char set_zda_disabled[] = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x07, 0x00, 0x01, 0x1D, 0x0D, 0x0A};
    mcu_utils_uart2_write((uint8_t*)set_zda_disabled, sizeof(set_zda_disabled), true);
    mcu_utils_delay_ms(500, is_rtos);

    // set GPS L1, BDS B1 enabled
    const char set_satellite_system[] = {0xF1, 0xD9, 0x06, 0x0C, 0x04, 0x00, 0x05, 0x00, 0x00, 0x00, 0x1B, 0xB0, 0x0D, 0x0A};
    mcu_utils_uart2_write((uint8_t*)set_satellite_system, sizeof(set_satellite_system), true);
    mcu_utils_delay_ms(500, is_rtos);
}
